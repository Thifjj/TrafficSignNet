...
`benchmark_example/benchmark_cpu_model_and_host.py`. It loads
`modelo/TrafficSignNet_INT8.h5` using the Vitis quantization scope and measures:
...
```bash
python3 benchmark_vitisai/benchmark_cpu_model_and_host.py
```
...
```bash
python3 benchmark_vitisai/benchmark_nvidia_gpu.py \
  --warmup 2 --runs 1 --seconds 1
```
...
```bash
python3 benchmark_vitisai/benchmark_nvidia_gpu.py --gpu-index 0
```
